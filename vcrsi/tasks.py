...
# =========================================================================== #
# THE EXTERNAL EMERGENCE-MEASUREMENT HELD-OUT SET (§4) -- SEALED.               #
# --------------------------------------------------------------------------- #
# Human-authored, never generated by the open-ended generator and never trained #
# on during the loop (kept out of SUITE -> verifier_fp is unchanged; the         #
# generator imports neither this list nor tasks.py at all). Each is on the §6A   #
# whitelist, clears §6B, and is behaviourally DISTINCT from every suite task AND  #
# from the generator's templates (asserted by emergence_set_is_sealed). The      #
# emergence delta = how many of THESE a self-generation-trained system solves     #
# minus a fixed-suite-trained baseline -- capability on tasks it was never shown. #
# =========================================================================== #
def _ext_expand() -> Node:
    return b("map", arg(0, "L"), b("srepeat", b("fst", it()), b("snd", it())))


def _ext_rle_thrice() -> Node:
    e = _ext_expand()
    return b("sconcat", b("lapp", b("lapp", e, e), e))


def _ext_decode_sortrev() -> Node:
    src = b("lrev", b("lsort", arg(0, "L")))
    return b("sconcat", b("map", src, b("srepeat", b("fst", it()), b("snd", it()))))


def _ext_span_scaled() -> Node:           # (fst+snd)*k for each interval
    return b("map", arg(0, "L"),
             b("mul", b("add", b("fst", it()), b("snd", it())), arg(1, "I")))


def _ext_double_width() -> Node:          # 2*width + 1
    return b("map", arg(0, "L"),
             b("add", b("mul", b("sub", b("snd", it()), b("fst", it())), lit(2)),
               lit(1)))


def _ext_mirror() -> Node:                # reflect each interval through 0
    return b("map", arg(0, "L"),
             b("pair", b("sub", lit(0), b("snd", it())),
               b("sub", lit(0), b("fst", it()))))


def _ext_contains_zero() -> Node:         # keep intervals that straddle 0
    return b("filter", arg(0, "L"),
             b("and", b("le", b("fst", it()), lit(0)),
               b("le", lit(0), b("snd", it()))))


def _ext_caesar3() -> Node:               # fixed +3 codepoint shift (no knob)
    return b("sconcat", b("map", b("schars", arg(0, "S")),
...
EMERGENCE_SET: List[Task] = [
    Task("ext_rle_thrice", 2, "Run-length decode and output the decoded string "
         "three times.", ("L",), "S", _ext_rle_thrice(), _gen_pairs,
         group="seqcode", note="external held-out"),
    Task("ext_decode_sortrev", 2, "Sort the (char,count) pairs, reverse, then "
         "run-length decode.", ("L",), "S", _ext_decode_sortrev(), _gen_pairs,
         group="seqcode", note="external held-out"),
    Task("ext_span_scaled", 4, "Map each interval to (a+b) scaled by arg1.",
         ("L", "I"), "L", _ext_span_scaled(), _gen_ivk,
         group="project", note="external held-out"),
    Task("ext_double_width", 4, "Map each interval to twice its width plus one.",
         ("L",), "L", _ext_double_width(), _gen_iv,
         group="project", note="external held-out"),
    Task("ext_mirror", 4, "Reflect every interval (a,b) through 0 -> (-b,-a).",
         ("L",), "L", _ext_mirror(), _gen_iv,
         group="interval", note="external held-out"),
    Task("ext_contains_zero", 3, "Keep only intervals that contain the point 0.",
         ("L",), "L", _ext_contains_zero(), _gen_iv,
         group="select", note="external held-out (frontier)"),
...
EMERGENCE_BY_NAME = {t.name: t for t in EMERGENCE_SET}
